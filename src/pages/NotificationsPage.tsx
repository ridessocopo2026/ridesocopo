...
import { Bell, CheckCheck, Car, Wallet, MapPin, User, Banknote, ClipboardCheck, Star, Info, ChevronRight, RefreshCw, BellOff, BellRing, Trash2 } from 'lucide-react'
...
function dayLabel(date: string): string {
  ...
  return target.toLocaleDateString('es-VE', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
  })
...
export function NotificationsPage() {
  ...
  const handleClearAll = async () => {
    if (!window.confirm('¿Vaciar todas las notificaciones? Esta acción no se puede deshacer.')) {
      return
    }
    setActing(true)
    try {
  ...
  // Al entrar a la bandeja, sincroniza con la base: las ofertas de viaje que
  // se purgaron (viaje ya aceptado/cancelado) desaparecen al instante.
  useEffect(() => {
    refreshNotifications()
  }, [refreshNotifications])
        ...
        {notifications.length > 0 && (
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={acting || !notifications.some((n) => n.is_read)}
              className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 text-xs font-semibold rounded-xl bg-surface-100 text-surface-600 hover:bg-surface-200 transition-colors disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={acting}
              className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 text-xs font-semibold rounded-xl bg-red-50 text-red-600 hover:bg-red-100 transition-colors disabled:opacity-50"
            >
              <Trash2 className="w-3.5 h-3.5" />
              Vaciar todo
            </button>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-2">
            {notifications.map((n, idx) => {
              ...
              const showDay = !prev || dayKey(n.created_at) !== dayKey(prev.created_at)
              return (
                <Fragment key={n.id}>
                  {showDay && (
                    <div className="flex items-center gap-2 pt-2 first:pt-0">
                      <span className="text-[11px] font-bold uppercase tracking-wider text-surface-400 bg-surface-100 rounded-full px-2.5 py-0.5">
                  ...
                  <div
                    ...
                    className={`card card-hover w-full text-left flex items-start gap-3 p-4 transition-all cursor-pointer ${
            ...
            {hasMore && (
              <button
                ...
                className="mt-1 w-full flex items-center justify-center gap-2 py-2.5 text-xs font-semibold text-primary-600 bg-primary-50 hover:bg-primary-100 rounded-xl transition-colors disabled:opacity-60"
              >
